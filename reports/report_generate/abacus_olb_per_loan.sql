IF OBJECT_ID('abacus_olb_per_loan') IS NOT NULL
    DROP FUNCTION abacus_olb_per_loan
GO

CREATE FUNCTION abacus_olb_per_loan
(	
	@dateFilter datetime,
...
(
	SELECT * ,
		...
		SELECT  CULoan.CustomerID as 'CustomerID',
		  ...
		  --          else CULoanPart.TermPeriodNum
			 ...
			 --   dbo._GetInitialGracePeriod_OTJ(CUAccount.CUAccountID) as 'GracePeriod_RSCHD',
				...
				[dbo].[OTJ_GetLateDays](CUAccount.CUAccountID, @dateFilter) AS 'Late Days',
				...
				[dbo].[_AmountSizeClassOperationDep](dbo._GetIssueAmount_OTJ(CULoan.CUAccountID) * [dbo].OTJ_GetXR(Currency.CurrencyID, 18, dbo._GetDisbusmentDate_OTJ(CULoan.CUAccountID)),18) AS 'Loan size from Dis Oper TJS',
				[dbo].[_AmountSizeClassOperationDep](dbo._BalanceAtDate(CUAccount.CUAccountID, @dateFilter)* [dbo].OTJ_GetXR(Currency.CurrencyID, 18, @dateFilter),18) AS 'Loan size from OLB Oper TJS',
		...
		where 
			...
			--(@currencyFilter =0  or PreparedData.CurrencyID=@currencyFilter)
			--and PreparedData.BranchName  in (@branchFilter)
			...
			--and PreparedData.[Contract Code]='029/040064587/GL_CON_TJS'

		--ORDER BY  PreparedData.[Start Date]
)
GO
